...
class DBRepl:
    ...
    async def handle(self, line: str) -> bool:
        ...
        if first == "help":
            ...
            table.add_row(
                "exit / quit",
                "Close the session and exit the application",
                "exit",
            )
            table.add_row(
                "<SQL Command>",
        ...
        if first == "connect":
            if len(parts) < 3:
                console.print("[red]connect <postgres|mysql|mongo> <url>[/red]")
                return True
            ...
            try:
                ...
                console.print(f"[red]ERROR of connection: {e}[/red]")
                return True
            self.adapter = adapter
            self.kind = kind
            console.print(f"[green]CONNECTION: {kind}[/green]")
            return True
        ...
        if self.adapter is None:
            console.print("[red]Connect firstly[/red]")
            return True
